...
class Embedding(nn.Module):
    ...
    def forward(self, x):
        B, P, L = x.shape   #Batch size, number of features per time step and sequence length
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # qkv [batch_size*(length+1), 3 * embed_dim]
        ...
        # transpose [batch_size, num_heads, length+1, embed_dim_per_head] -> [batch_size, length+1, num_heads, embed_dim_per_head]
        # reshape [batch_size, length+1, num_heads, embed_dim_per_head] - > [batch_size, length+1, embed_dim]
        # reshape [batch_size, length+1, embed_dim] -> [batch_size*(length+1), embed_dim]
